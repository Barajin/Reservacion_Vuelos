CREATE DATABASE reservación_vuelos;
GO

USE reservación_vuelos;
GO

CREATE TABLE ciudad (
  cveCiudad CHAR(3) PRIMARY KEY,
  nombreCiudad VARCHAR(20) NOT NULL,
  estado VARCHAR(20) NOT NULL
)

CREATE TABLE vuelo (
  cveVuelo INT IDENTITY(1,1) PRIMARY KEY,
  numPasajeros INT NOT NULL,
  origen CHAR(3) NOT NULL FOREIGN KEY REFERENCES ciudad(cveCiudad),
  destino CHAR(3) NOT NULL FOREIGN KEY REFERENCES ciudad(cveCiudad),
  capacidad INT NOT NULL CHECK(capacidad>0),
  costo DECIMAL(10,3) NOT NULL CHECK(costo>0),
  millas DECIMAL(10,3) NOT NULL CHECK(millas>0),
)

CREATE TABLE días (
	cveVuelo INT FOREIGN KEY REFERENCES vuelo(cveVuelo),
	díaVuelo VARCHAR(10) CHECK(díaVuelo IN ('Lunes','Martes','Miércoles','Jueves','Viernes','Sábado','Domingo','Diaria')),
	PRIMARY KEY (cveVuelo, díaVuelo)
)

CREATE TABLE cliente (
  cveCliente INT IDENTITY(1,1) PRIMARY KEY,
  nombre VARCHAR(50) NOT NULL,
  edad INT NOT NULL CHECK(edad>0)
)

CREATE TABLE club_premier (
  cveClubPremier INT IDENTITY(1,1) PRIMARY KEY,
  cveCliente INT NOT NULL FOREIGN KEY REFERENCES cliente(cveCliente),
  domicilio VARCHAR(60) NOT NULL
)

CREATE TABLE clienteGenérico(
	cveCliente INT PRIMARY KEY FOREIGN KEY REFERENCES cliente(cveCliente)
)
  
CREATE TABLE boleto (
  cveBoleto INT IDENTITY(1,1) PRIMARY KEY,
  cveVuelo INT FOREIGN KEY REFERENCES vuelo(cveVuelo),
  cveCliente INT FOREIGN KEY REFERENCES cliente(cveCliente)
)
INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('AGS','Aguascalientes','Aguascalientes')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('MEC','Mexicali','Baja California')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('LAA','La Paz','Baja California Sur')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('CAE','Campeche','Campeche')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('SAI','Saltillo','Coahuila')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('COM','Colima','Colima')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('TUL','Tuxtla Gutiérrez','Chiapas')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('CHU','Chihuahua','Chihuahua')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('CIA','Ciudad de México','Distrito Federal')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('DUN','Durango','Durango')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('GUA','Guanajuato','Guanajuato')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('CHP','Chilpancingo','Guerrero')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('PAU','Pachuca','Hidalgo')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('GUJ','Guadalajara','Jalisco')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('TOC','Toluca','México')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('MOL','Morelia','Michoacán')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('CUN','Cuernavaca','Morelos')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('TEC','Tepic','Narayit')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('MOE','Monterrey','Nuevo León')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('OAC','Oaxaca','Oaxaca')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('PUL','Puebla','Puebla')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('QUÉ','Querétaro','Querétaro')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('CHT','Chetumal','Quintana Roo')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('SAL','San Luis Potosí','San Luis Potosí')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('CUA','Culiacán','Sinaloa')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('HEO','Hermosillo','Sonora')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('VIA','Villahermosa','Tabasco')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('CIV','Ciudad Victoria','Tamaulipas')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('TLC','Tlaxcala','Tlaxcala')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('XAP','Xalapa','Veracruz')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('MÉD','Mérida','Yucatán')

INSERT INTO ciudad(cveCiudad, nombreCiudad, estado)
   VALUES('ZAT','Zacatecas','Zacatecas')

INSERT INTO cliente(nombre, edad)
   VALUES('Fernando Gálvez', 70)

INSERT INTO cliente(nombre, edad)
   VALUES('Roberto Herondale', 34)

INSERT INTO cliente(nombre, edad)
   VALUES('Daniel Preciado', 85)

INSERT INTO cliente(nombre, edad)
   VALUES('Diego Vidriales', 77)

INSERT INTO cliente(nombre, edad)
   VALUES('Daniel González', 72)

INSERT INTO cliente(nombre, edad)
   VALUES('Eduardo Gallardo', 5)

INSERT INTO cliente(nombre, edad)
   VALUES('Karen Gálvez', 36)

INSERT INTO cliente(nombre, edad)
   VALUES('Eduardo Pulido', 49)

INSERT INTO cliente(nombre, edad)
   VALUES('Karen Herondale', 11)

INSERT INTO cliente(nombre, edad)
   VALUES('Fernando Apodaca', 11)

INSERT INTO cliente(nombre, edad)
   VALUES('Emma Bajo', 66)

INSERT INTO cliente(nombre, edad)
   VALUES('Fernando Ortíz', 86)

INSERT INTO cliente(nombre, edad)
   VALUES('Emma Gallardo', 89)

INSERT INTO cliente(nombre, edad)
   VALUES('Roberto Vidriales', 33)

INSERT INTO cliente(nombre, edad)
   VALUES('Melisse González', 67)

INSERT INTO cliente(nombre, edad)
   VALUES('Roberto Medina', 11)

INSERT INTO cliente(nombre, edad)
   VALUES('Melisse Apodaca', 45)

INSERT INTO cliente(nombre, edad)
   VALUES('Pedro Bajo', 44)

INSERT INTO cliente(nombre, edad)
   VALUES('René Herondale', 6)

INSERT INTO cliente(nombre, edad)
   VALUES('Pedro Preciado', 41)

INSERT INTO cliente(nombre, edad)
   VALUES('René Vidriales', 79)

INSERT INTO cliente(nombre, edad)
   VALUES('Jesús González', 59)

INSERT INTO cliente(nombre, edad)
   VALUES('Diego Gallardo', 19)

INSERT INTO cliente(nombre, edad)
   VALUES('Jesús Gálvez', 69)

INSERT INTO cliente(nombre, edad)
   VALUES('Diego Pulido', 79)

INSERT INTO cliente(nombre, edad)
   VALUES('Daniel Herondale', 64)

INSERT INTO cliente(nombre, edad)
   VALUES('Eduardo Preciado', 26)

INSERT INTO cliente(nombre, edad)
   VALUES('Daniel Bajo', 63)

INSERT INTO cliente(nombre, edad)
   VALUES('Eduardo Ortíz', 47)

INSERT INTO cliente(nombre, edad)
   VALUES('Karen Gallardo', 50)

INSERT INTO cliente(nombre, edad)
   VALUES('Karen González', 7)

INSERT INTO cliente(nombre, edad)
   VALUES('Fernando Medina', 48)

INSERT INTO cliente(nombre, edad)
   VALUES('Emma Apodaca', 38)

INSERT INTO cliente(nombre, edad)
   VALUES('Roberto Bajo', 23)

INSERT INTO cliente(nombre, edad)
   VALUES('Emma Herondale', 41)

INSERT INTO cliente(nombre, edad)
   VALUES('Roberto Preciado', 53)

INSERT INTO cliente(nombre, edad)
   VALUES('Melisse Vidriales', 54)

INSERT INTO cliente(nombre, edad)
   VALUES('Pedro González', 21)

INSERT INTO cliente(nombre, edad)
   VALUES('Melisse Medina', 43)

INSERT INTO cliente(nombre, edad)
   VALUES('Pedro Gálvez', 17)

INSERT INTO cliente(nombre, edad)
   VALUES('René Pulido', 62)

INSERT INTO cliente(nombre, edad)
   VALUES('Jesús Herondale', 13)

INSERT INTO cliente(nombre, edad)
   VALUES('René Preciado', 21)

INSERT INTO cliente(nombre, edad)
   VALUES('Jesús Bajo', 74)

INSERT INTO cliente(nombre, edad)
   VALUES('Diego Ortíz', 9)

INSERT INTO cliente(nombre, edad)
   VALUES('Daniel Gallardo', 15)

INSERT INTO cliente(nombre, edad)
   VALUES('Diego Gálvez', 77)

INSERT INTO cliente(nombre, edad)
   VALUES('Daniel Pulido', 40)

INSERT INTO cliente(nombre, edad)
   VALUES('Eduardo Medina', 71)

INSERT INTO cliente(nombre, edad)
   VALUES('Karen Apodaca', 77)

INSERT INTO cliente(nombre, edad)
   VALUES('Eduardo Bajo', 6)

INSERT INTO cliente(nombre, edad)
   VALUES('Karen Ortíz', 11)

INSERT INTO cliente(nombre, edad)
   VALUES('Fernando Preciado', 81)

INSERT INTO cliente(nombre, edad)
   VALUES('Emma Vidriales', 46)

INSERT INTO cliente(nombre, edad)
   VALUES('Fernando González', 30)

INSERT INTO cliente(nombre, edad)
   VALUES('Emma Medina', 15)

INSERT INTO cliente(nombre, edad)
   VALUES('Roberto Gálvez', 20)

INSERT INTO cliente(nombre, edad)
   VALUES('Melisse Pulido', 20)

INSERT INTO cliente(nombre, edad)
   VALUES('Melisse Preciado', 87)

INSERT INTO cliente(nombre, edad)
   VALUES('Pedro Vidriales', 12)

INSERT INTO cliente(nombre, edad)
   VALUES('René Ortíz', 68)

INSERT INTO cliente(nombre, edad)
   VALUES('Pedro Gallardo', 63)

INSERT INTO cliente(nombre, edad)
   VALUES('René Gálvez', 6)

INSERT INTO cliente(nombre, edad)
   VALUES('Jesús Pulido', 51)

INSERT INTO cliente(nombre, edad)
   VALUES('Diego Medina', 9)

INSERT INTO cliente(nombre, edad)
   VALUES('Jesús Apodaca', 23)

INSERT INTO cliente(nombre, edad)
   VALUES('Diego Bajo', 53)

INSERT INTO cliente(nombre, edad)
   VALUES('Daniel Ortíz', 70)

INSERT INTO cliente(nombre, edad)
   VALUES('Daniel Vidriales', 81)

INSERT INTO cliente(nombre, edad)
   VALUES('Eduardo González', 18)

INSERT INTO cliente(nombre, edad)
   VALUES('Karen Medina', 47)

INSERT INTO cliente(nombre, edad)
   VALUES('Eduardo Apodaca', 28)

INSERT INTO cliente(nombre, edad)
   VALUES('Karen Pulido', 5)

INSERT INTO cliente(nombre, edad)
   VALUES('Fernando Herondale', 79)

INSERT INTO cliente(nombre, edad)
   VALUES('Emma Preciado', 68)

INSERT INTO cliente(nombre, edad)
   VALUES('Fernando Vidriales', 70)

INSERT INTO cliente(nombre, edad)
   VALUES('Emma Ortíz', 42)

INSERT INTO cliente(nombre, edad)
   VALUES('Roberto Gallardo', 34)

INSERT INTO cliente(nombre, edad)
   VALUES('Melisse Gálvez', 64)

INSERT INTO cliente(nombre, edad)
   VALUES('Roberto Pulido', 69)

INSERT INTO cliente(nombre, edad)
   VALUES('Melisse Herondale', 77)

INSERT INTO cliente(nombre, edad)
   VALUES('Pedro Apodaca', 76)

INSERT INTO cliente(nombre, edad)
   VALUES('René Bajo', 81)

INSERT INTO cliente(nombre, edad)
   VALUES('Pedro Ortíz', 60)

INSERT INTO cliente(nombre, edad)
   VALUES('René Gallardo', 49)

INSERT INTO cliente(nombre, edad)
   VALUES('Jesús Vidriales', 85)

INSERT INTO cliente(nombre, edad)
   VALUES('Diego González', 56)

INSERT INTO cliente(nombre, edad)
   VALUES('Jesús Medina', 82)

INSERT INTO cliente(nombre, edad)
   VALUES('Diego Apodaca', 83)

INSERT INTO cliente(nombre, edad)
   VALUES('Eduardo Herondale', 30)

INSERT INTO cliente(nombre, edad)
   VALUES('Eduardo Vidriales', 37)

INSERT INTO cliente(nombre, edad)
   VALUES('Fernando Gallardo', 38)

INSERT INTO cliente(nombre, edad)
   VALUES('Fernando Pulido', 23)

INSERT INTO cliente(nombre, edad)
   VALUES('Roberto Apodaca', 36)

INSERT INTO cliente(nombre, edad)
   VALUES('Roberto Ortíz', 38)

INSERT INTO cliente(nombre, edad)
   VALUES('Melisse Gallardo', 89)

INSERT INTO cliente(nombre, edad)
   VALUES('Pedro Medina', 11)

INSERT INTO cliente(nombre, edad)
   VALUES('René Apodaca', 60)

INSERT INTO cliente(nombre, edad)
   VALUES('Jesús Preciado', 70)

INSERT INTO cliente(nombre, edad)
   VALUES('Daniel Gálvez', 76)

INSERT INTO cliente(nombre, edad)
   VALUES('Karen Bajo', 52)

INSERT INTO cliente(nombre, edad)
   VALUES('Emma González', 11)

INSERT INTO cliente(nombre, edad)
   VALUES('Melisse Ortíz', 48)

INSERT INTO cliente(nombre, edad)
   VALUES('René Medina', 57)

INSERT INTO cliente(nombre, edad)
   VALUES('Diego Preciado', 34)

INSERT INTO cliente(nombre, edad)
   VALUES('Eduardo Gálvez', 67)

INSERT INTO cliente(nombre, edad)
   VALUES('Fernando Bajo', 10)

INSERT INTO cliente(nombre, edad)
   VALUES('Roberto González', 7)

INSERT INTO cliente(nombre, edad)
   VALUES('Pedro Herondale', 68)

INSERT INTO cliente(nombre, edad)
   VALUES('Jesús Gallardo', 49)

INSERT INTO cliente(nombre, edad)
   VALUES('Daniel Apodaca', 63)

INSERT INTO cliente(nombre, edad)
   VALUES('Karen Vidriales', 76)

INSERT INTO cliente(nombre, edad)
   VALUES('Emma Pulido', 46)

INSERT INTO cliente(nombre, edad)
   VALUES('Pedro Pulido', 23)

INSERT INTO cliente(nombre, edad)
   VALUES('Jesús Ortíz', 24)

INSERT INTO cliente(nombre, edad)
   VALUES('Daniel Medina', 82)

INSERT INTO cliente(nombre, edad)
   VALUES('Karen Preciado', 71)

INSERT INTO cliente(nombre, edad)
   VALUES('Emma Gálvez', 36)

INSERT INTO cliente(nombre, edad)
   VALUES('Melisse Bajo', 7)

INSERT INTO cliente(nombre, edad)
   VALUES('René González', 13)

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(60, 'Tierra, Nuevo Culiacán #2287')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(55, 'Río Acaponeta, Popular #3762')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(21, 'Río Aguanaval, Infonavit Humaya #337')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(35, 'Río Acaponeta, Los Pinos #1812')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(45, 'Río Aguanaval, Popular #3287')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(89, 'Río Lerma, Infonavit Humaya #4762')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(19, 'Río Aguanaval, Los Pinos #1337')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(47, 'Río Lerma, Industrial Bravo #2812')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(43, 'Río Suchiate, Las Quintas #4287')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(17, 'Río Lerma, Guadalupe #862')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(102, 'Río Suchiate, Nuevo Culiacán #2337')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(88, 'Av. Guadalupe Victoria, Popular #3812')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(96, 'Río Suchiate, Infonavit Humaya #387')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(26, 'Av. Guadalupe Victoria, Los Pinos #1862')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(103, 'Juan de Dios Bátiz, Industrial Bravo #3337')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(39, 'Av. Guadalupe Victoria, Las Quintas #4812')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(84, 'Juan de Dios Bátiz, Guadalupe #1387')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(108, 'Tierra, Nuevo Culiacán #2862')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(61, 'Juan de Dios Bátiz, Popular #4337')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(56, 'Tierra, Infonavit Humaya #911')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(77, 'Río Acaponeta, Los Pinos #2386')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(42, 'Tierra, Industrial Bravo #3861')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(4, 'Río Acaponeta, Las Quintas #436')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(97, 'Río Aguanaval, Los Pinos #1911')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(64, 'Río Acaponeta, Industrial Bravo #3386')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(107, 'Río Aguanaval, Las Quintas #4861')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(91, 'Río Lerma, Guadalupe #1436')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(24, 'Río Aguanaval, Nuevo Culiacán #2911')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(9, 'Río Lerma, Popular #4386')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(31, 'Río Aguanaval, Infonavit Humaya #961')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(75, 'Río Lerma, Los Pinos #2436')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(95, 'Río Suchiate, Industrial Bravo #3911')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(63, 'Río Lerma, Las Quintas #486')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(25, 'Río Suchiate, Guadalupe #1961')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(93, 'Av. Guadalupe Victoria, Nuevo Culiacán #3436')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(23, 'Río Suchiate, Popular #4911')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(52, 'Av. Guadalupe Victoria, Infonavit Humaya #1486')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(10, 'Juan de Dios Bátiz, Los Pinos #2961')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(33, 'Av. Guadalupe Victoria, Industrial Bravo #4436')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(62, 'Juan de Dios Bátiz, Las Quintas #1011')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(78, 'Tierra, Guadalupe #2486')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(81, 'Juan de Dios Bátiz, Industrial Bravo #3961')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(72, 'Tierra, Las Quintas #536')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(53, 'Río Acaponeta, Guadalupe #2011')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(18, 'Tierra, Nuevo Culiacán #3485')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(51, 'Río Acaponeta, Popular #4960')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(48, 'Río Aguanaval, Infonavit Humaya #1535')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(104, 'Río Acaponeta, Los Pinos #3010')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(92, 'Río Aguanaval, Industrial Bravo #4485')

INSERT INTO club_premier(cveCliente, domicilio)
   VALUES(38, 'Río Lerma, Las Quintas #1060')

INSERT INTO clienteGenérico(cveCliente)
   VALUES(118)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(11)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(82)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(6)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(22)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(99)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(111)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(2)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(37)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(14)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(16)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(116)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(20)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(59)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(46)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(58)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(98)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(73)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(113)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(30)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(112)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(110)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(76)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(27)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(80)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(106)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(50)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(69)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(5)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(68)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(114)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(32)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(109)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(119)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(13)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(86)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(54)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(36)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(29)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(83)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(40)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(7)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(71)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(117)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(44)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(15)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(74)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(70)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(67)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(49)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(3)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(100)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(105)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(87)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(101)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(41)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(66)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(12)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(85)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(79)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(115)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(94)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(28)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(8)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(57)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(34)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(65)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(90)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(1)

INSERT INTO clienteGenérico(cveCliente)
   VALUES(120)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(19,'HEO','CIA',46,2380.98,1999.19)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(36,'OAC','TLC',46,4135.56,3741.2)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(18,'CAE','OAC',25,1130.43,1714.33)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(40,'MOE','GUA',45,2424.9,4177.45)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(44,'CUN','SAL',45,713.64,123.71)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(26,'AGS','TOC',25,2007.52,2585.83)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(48,'MOL','COM',45,3301.99,558.96)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(30,'MÉD','CIA',25,296.86,3021.9)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(13,'GUA','MOE',45,1590.73,994.21)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(35,'TLC','DUN',25,2884.6,3457.34)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(17,'PAU','MÉD',45,4179.48,1430.46)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(39,'CIA','QUÉ',25,1173.95,3892.59)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(21,'CHP','PAU',45,2468.82,1865.72)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(43,'HEO','CAE',25,3762.69,4328.84)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(25,'DUN','HEO',45,756.56,2301.97)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(47,'CUA','CUN',25,2051.44,274.9)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(29,'CIA','CHU',44,3345.91,2737.22)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(11,'SAL','XAP',24,339.78,709.35)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(33,'TUL','PUL',44,1634.65,3172.47)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(15,'QUÉ','CHP',24,2928.52,1145.6)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(38,'COM','MEC',44,4222.99,3608.72)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(20,'PUL','SAL',24,1217.87,1581.85)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(42,'SAI','TOC',44,2511.74,4044.98)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(24,'MOE','COM',24,3805.61,2016.1)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(46,'LAA','CIA',44,800.48,4479.23)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(28,'TEC','MOE',24,2094.95,2452.36)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(10,'MEC','DUN',44,3388.83,425.48)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(32,'CUN','AGS',24,383.7,2888.61)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(14,'AGS','CHU',44,1677.57,861.73)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(36,'TOC','GUA',24,2971.44,3323.86)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(18,'XAP','CAE',43,4266.91,1296.99)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(40,'GUA','HEO',23,1260.79,3759.11)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(22,'TLC','CUN',43,2555.66,1732.24)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(45,'PAU','CHU',23,3849.53,4195.36)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(27,'VIA','XAP',43,843.4,2168.49)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(49,'GUA','PUL',23,2138.87,140.62)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(31,'HEO','CHP',43,3432.75,2603.74)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(13,'DUN','LAA',23,426.62,576.87)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(17,'CHU','MOL',23,3015.96,1012.12)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(39,'CHU','TUL',43,4309.83,3474.25)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(21,'TUL','CIA',23,1304.71,1447.37)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(43,'QUÉ','MOE',43,2598.58,3910.5)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(25,'COM','DUN',23,3892.45,1883.63)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(47,'OAC','AGS',43,887.92,4346.75)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(29,'CAE','CHU',22,2181.79,2319.88)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(12,'MOE','GUA',42,3475.67,291)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(34,'LAA','SAI',22,470.54,2754.13)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(16,'TEC','VIA',42,1764.41,727.26)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(38,'AGS','TEC',22,3059.88,3190.38)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(20,'MOL','CIA',42,4353.75,1163.51)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(42,'MÉD','XAP',22,1347.62,3626.63)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(24,'TOC','PUL',42,2642.5,1598.76)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(46,'XAP','CHP',22,3936.97,4061.89)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(28,'PAU','LAA',42,930.84,2034.1)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(10,'CIA','CUA',22,2225.71,4497.14)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(32,'CHP','MOL',42,3519.58,2470.26)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(14,'VIA','TUL',22,513.46,443.39)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(36,'GUA','TLC',42,1808.93,2905.52)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(19,'CUA','OAC',22,3102.8,878.64)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(41,'CIA','GUA',41,4396.67,3341.77)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(23,'SAL','MEC',21,1391.54,1314.9)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(45,'CHU','CHU',41,2685.42,3777.2)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(27,'CHU','GUA',21,3979.89,1750.15)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(49,'COM','SAI',41,974.76,4212.27)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(31,'PUL','VIA',21,2268.63,2185.4)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(13,'SAI','TEC',41,3563.5,158.53)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(35,'OAC','CIA',21,557.98,2621.65)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(17,'CAE','MÉD',41,1851.85,594.78)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(39,'MOE','QUÉ',21,3146.72,3057.9)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(21,'MEC','PAU',41,4440.59,1029.3)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(43,'CUN','CAE',21,1434.46,3492.16)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(26,'AGS','CUA',41,2729.94,1465.28)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(30,'MÉD','TUL',41,1017.68,1901.53)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(12,'GUA','TLC',20,2312.55,4363.66)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(34,'TLC','OAC',40,3606.42,2336.79)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(16,'PAU','GUA',20,600.9,309.91)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(38,'CIA','MEC',40,1895.77,2772.4)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(20,'CHP','SAL',20,3189.64,745.16)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(42,'HEO','TOC',40,4483.51,3208.29)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(24,'DUN','COM',20,1478.98,1180.42)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(46,'CUA','CIA',40,2772.85,3643.54)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(28,'CIA','TEC',20,4066.73,1616.67)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(10,'SAL','CIA',40,1061.6,4079.8)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(33,'TUL','MÉD',20,2355.47,2052.92)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(37,'COM','PAU',20,644.81,2487.17)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(19,'PUL','CAE',40,1938.69,460.3)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(41,'SAI','HEO',20,3233.56,2923.43)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(23,'MOE','CUN',39,227.43,896.55)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(45,'LAA','CHU',19,1521.9,3359.68)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(27,'TEC','XAP',39,2816.77,1332.8)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(49,'MEC','OAC',19,4110.65,3794.93)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(31,'CUN','GUA',39,1104.52,1767.6)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(13,'MÉD','MEC',19,2399.99,4230.18)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(35,'TOC','SAL',39,3693.86,2203.31)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(17,'XAP','TOC',19,687.73,176.43)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(40,'GUA','COM',39,1982.61,2639.56)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(22,'TLC','CIA',19,3276.48,611.69)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(44,'CHP','MOE',39,270.95,3074.81)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(26,'VIA','DUN',19,1565.82,1047.94)

INSERT INTO vuelo(numPasajeros, origen, destino, capacidad, costo, millas)
   VALUES(48,'GUA','AGS',39,2859.69,3510.7)

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(1, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(2, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(3, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(4, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(5, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(6, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(7, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(8, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(9, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(10, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(11, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(12, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(13, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(14, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(15, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(16, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(17, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(18, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(19, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(20, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(21, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(22, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(23, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(24, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(25, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(26, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(27, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(28, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(29, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(30, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(31, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(32, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(33, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(34, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(35, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(36, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(37, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(38, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(39, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(40, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(41, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(42, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(43, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(44, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(45, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(46, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(47, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(48, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(49, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(50, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(51, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(52, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(53, 'Sábado')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(54, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(55, 'Sábado')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(56, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(57, 'Sábado')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(58, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(59, 'Sábado')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(60, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(61, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(62, 'Miércoles')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(63, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(64, 'Miércoles')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(65, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(66, 'Miércoles')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(67, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(68, 'Miércoles')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(69, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(70, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(71, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(72, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(73, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(74, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(75, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(76, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(77, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(78, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(79, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(80, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(81, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(82, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(83, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(84, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(85, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(86, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(87, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(88, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(89, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(90, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(91, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(92, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(93, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(94, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(95, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(96, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(97, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(98, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(99, 'Miércoles')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(100, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(1, 'Miércoles')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(2, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(3, 'Miércoles')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(4, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(5, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(6, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(7, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(8, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(9, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(10, 'Miércoles')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(11, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(12, 'Miércoles')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(13, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(14, 'Miércoles')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(15, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(16, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(17, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(18, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(19, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(20, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(21, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(22, 'Sábado')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(23, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(24, 'Sábado')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(25, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(26, 'Sábado')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(27, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(28, 'Sábado')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(29, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(30, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(31, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(32, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(33, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(34, 'Sábado')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(35, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(36, 'Sábado')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(37, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(38, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(39, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(40, 'Sábado')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(41, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(42, 'Sábado')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(43, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(44, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(45, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(46, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(47, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(48, 'Sábado')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(49, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(50, 'Sábado')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(51, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(52, 'Miércoles')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(53, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(54, 'Miércoles')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(55, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(56, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(57, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(58, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(59, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(60, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(61, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(62, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(63, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(64, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(65, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(66, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(67, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(68, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(69, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(70, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(71, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(72, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(73, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(74, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(75, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(76, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(77, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(78, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(79, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(80, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(81, 'Miércoles')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(82, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(83, 'Miércoles')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(84, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(85, 'Diaria')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(86, 'Lunes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(87, 'Miércoles')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(88, 'Sábado')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(89, 'Miércoles')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(90, 'Sábado')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(91, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(92, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(93, 'Jueves')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(94, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(95, 'Miércoles')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(96, 'Miércoles')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(97, 'Martes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(98, 'Domingo')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(99, 'Viernes')

INSERT INTO días(cveVuelo, díaVuelo)
   VALUES(100, 'Domingo')

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(47,13)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(94,96)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(28,32)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(78,105)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(18,74)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(74,81)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(12,111)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(7,4)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(83,8)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(60,78)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(38,59)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(42,62)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(58,6)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(33,2)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(72,15)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(62,37)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(37,19)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(14,53)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(57,32)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(89,27)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(87,99)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(50,112)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(63,45)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(69,118)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(44,97)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(1,35)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(54,105)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(76,31)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(45,117)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(19,74)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(43,32)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(82,15)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(73,23)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(52,59)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(97,90)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(35,41)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(90,116)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(21,30)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(36,103)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(85,76)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(48,49)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(39,73)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(26,25)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(17,96)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(32,113)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(5,68)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(34,41)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(93,14)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(46,86)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(71,32)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(27,26)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(16,7)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(65,75)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(68,90)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(9,19)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(80,6)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(8,77)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(49,73)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(2,56)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(10,102)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(6,49)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(31,59)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(67,16)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(29,70)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(88,12)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(41,112)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(91,61)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(95,91)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(11,3)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(23,118)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(66,71)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(59,25)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(30,65)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(75,87)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(22,82)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(56,103)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(24,9)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(61,104)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(86,10)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(3,11)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(84,69)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(25,21)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(20,69)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(81,33)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(4,3)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(15,22)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(55,9)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(51,84)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(79,72)

INSERT INTO boleto(cveVuelo, cveCliente)
   VALUES(77,100)

   
    CREATE PROCEDURE [notRepeat]
    @cveClubPremier varchar (8)
   AS
   BEGIN 
	 SELECT cveClubPremier
	 FROM	reservación_vuelos
	 WHERE cveClubPremier= @cveClubPremier
   END

   
